...
/// Modelo de un ítem del ticket (no confundir con la entidad de base de datos)
class TicketItem {
  final String nombre;
...
/// Servicio para generar e imprimir tickets en PDF o impresora térmica
class TicketService {
  /// Método adaptador para recibir datos desde la UI y lanzar el proceso de generación de PDF/impresión
  ...
  /// Genera el documento PDF del ticket
  ...
  /// Procesa la impresión silenciosa si hay hardware conectado; de lo contrario abre el diálogo del sistema
  static Future<void> generarYProcesarPdf({
    ...
    // Respaldar PDF en disco
    _guardarEnDiscoSilencioso(bytes);

    // Impresión Silenciosa Directa vs. Diálogo del Sistema
    if (impresoraSeleccionada != null) {
      try {
        ...
        // Rasterizar el PDF a imágenes para máxima compatibilidad térmica
      ...
      // Respaldo mediante diálogo nativo
      await Printing.layoutPdf(
        onLayout: (PdfPageFormat format) async => bytes,
        name: 'Ticket_${DateTime.now().millisecondsSinceEpoch}',
      );
